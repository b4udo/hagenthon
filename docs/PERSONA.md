...
| **Non ha mai usato «cartelle» né «archivia»** | **Zero gerarchie**: una lista piatta e basta. Nessun tag, nessuna cartella, nessuna azione di archiviazione | `app/frontend/index.html` → un solo `<ul class="elenco">`; l'API espone `GET /api/mailbox`, non un albero |
