...
>
> **Chiude tutto.** Aspetta domenica, quando viene Guada.
...
|---|---|---|
| Non distingue una truffa da una comunicazione vera | **Semaforo di sicurezza** | 🔴 *«Non risponda»* + pulsante col **numero verde ufficiale**, preso da una tabella statica nel codice e **mai dall'email** · 🟡 *«Chieda a Guada»* + pulsante «Manda a Guada» · 🟢 *«Può rispondere»* + porta alla risposta guidata |
...
Nel corpus la rubrica di Maria contiene sei contatti: Guada (il nipote), i figli Anna e Paolo, un'amica, lo studio della dott.ssa Bianchi, la farmacia. Non è colore narrativo, ha due effetti tecnici precisi:
...
2. **La persona di fiducia dell'azione gialla ha un nome**: il pulsante dice *«Manda questa email a Guada»*, non *«inoltra»*. Il nome sta in una costante (`sicurezza.NOME_PERSONA_FIDUCIA`), non è ricavato dall'email.
