...
**Risultato su un clone pulito, con le sole dipendenze di `requirements.txt`: 73 passati, 1 saltato.**
...
Con Playwright installato quella suite gira davvero — avvia un `uvicorn` su `127.0.0.1` e percorre l'interfaccia in un Chromium — e il totale diventa **101 passati, 0 saltati**. Dettaglio di cosa copre ciascun file: [`docs/VALIDAZIONE.md`](docs/VALIDAZIONE.md).
